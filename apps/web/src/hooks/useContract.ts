import {
  ...
  export const useCake = (): { reader: Cake; signer: Cake } => {
    ...
    const signer = useProviderOrSigner(true)
    return useMemo(
      () => ({
        reader: getCakeContract(provider,ChainId.SHIB_TESTNET),
        signer: getCakeContract(signer,ChainId.SHIB_TESTNET),
      }),
      [signer],
    )
  }
  ...
  export function useBCakeFarmBoosterContract(withSignerIfPossible = true) {
    const providerOrSigner = useProviderOrSigner(withSignerIfPossible, false)
