...
const ContributeModal: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  const label = currency === shibTestnetTokens.cake ? t('Max. CAKE entry') : t('Max. token entry')
  ...
  const maximumTokenEntry = useMemo(() => {
    if (ifo.version >= 3.1 && poolId === PoolIds.poolBasic) {
  ...
  }, [userCurrencyBalance, limitPerUserInLP, amountTokenCommittedInLP, ifo.version, poolId])
              ...
              <Image
                src={
                  ifo.currency.symbol === 'CAKE'
                    ? '/images/cake.svg'
                    : `https://data.ball.exchange//images/tokens/${currency.symbol.split(' ')[0].toUpperCase()}.png`
                }
                width={24}
                height={24}
              />
