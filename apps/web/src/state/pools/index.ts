...
const cakeVaultAddress = getCakeVaultAddress(ChainId.SHIB_TESTNET)
...
export const fetchCakePoolUserDataAsync = (account: string) => async (dispatch) => {
  ...
  const allowanceCall = {
    address: shibTestnetTokens.cake.address,
    name: 'allowance',
    ...
    try {
      ...
      const bnbBusdFarm =
        activePriceHelperLpsConfig.length > 0
          ? farmsData.find((farm) => farm.token.symbol === 'BUSD' && farm.quoteToken.symbol === 'WBONE')
          : null
...
export const fetchCakeVaultFees = createAsyncThunk<SerializedVaultFees>('cakeVault/fetchFees', async () => {
  const vaultFees = await fetchVaultFees(getCakeVaultAddress(ChainId.SHIB_TESTNET))
  return vaultFees
})
...
export const fetchCakeFlexibleSideVaultFees = createAsyncThunk<SerializedVaultFees>(
  ...
  async () => {
    const vaultFees = await fetchVaultFees(getCakeFlexibleSideVaultAddress(ChainId.SHIB_TESTNET))
    return vaultFees
  },
)
