...
const startEndBlockCalls = livePoolsWithEnd.flatMap((poolConfig) => {
  ...
  return [
    {
      address: getAddress(poolConfig.contractAddress,ChainId.SHIB_TESTNET),
      name: 'startBlock',
    },
    {
      address: getAddress(poolConfig.contractAddress,ChainId.SHIB_TESTNET),
...
const poolsBalanceOf = poolsConfig.map((poolConfig) => {
  ...
  return {
    ...
    name: 'balanceOf',
    params: [getAddress(poolConfig.contractAddress,ChainId.SHIB_TESTNET)],
  }
})
 
export const fetchPoolsTotalStaking = async () => {

  const poolsTotalStaked = await multicall(erc20ABI, poolsBalanceOf,ChainId.SHIB_TESTNET)
...
export const fetchPoolsStakingLimits = async (
  ...
  const poolStakingCalls = validPools
    .map((validPool) => {
      const contractAddress = getAddress(validPool.contractAddress,ChainId.SHIB_TESTNET)
