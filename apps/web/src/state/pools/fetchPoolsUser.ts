...
const multicallAddress = getMulticallAddress(ChainId.SHIB_TESTNET)
export const fetchPoolsAllowance = async (account) => {
  const calls = nonBnbPools.map((pool) => ({
    address: pool.stakingToken.address,
    name: 'allowance',
    params: [account, getAddress(pool.contractAddress,ChainId.SHIB_TESTNET)],
  }))

  const allowances = await multicall(erc20ABI, calls,ChainId.SHIB_TESTNET)
...
export const fetchUserBalances = async (account) => {
  ...
  const tokenBnbBalancesRaw = await multicallv3({ calls: [...tokenBalanceCalls, bnbBalanceCall] ,chainId:ChainId.SHIB_TESTNET})
...
export const fetchUserStakeBalances = async (account) => {
  ...
  const calls = nonMasterPools.map((p) => ({
    address: getAddress(p.contractAddress,ChainId.SHIB_TESTNET),
    name: 'userInfo',
    params: [account],
  }))

  const userInfo = await multicall(sousChefABI, calls,ChainId.SHIB_TESTNET)
  return fromPairs(
...
export const fetchUserPendingRewards = async (account) => {
  ...
  const res = await multicall(sousChefABI, calls,ChainId.SHIB_TESTNET)
