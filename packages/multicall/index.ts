...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.SHIB_TESTNET) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.SHIB_TESTNET, options, provider: _provider }) => { 
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.SHIB_TESTNET, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
