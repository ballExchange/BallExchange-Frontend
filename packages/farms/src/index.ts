...
const supportedChainId = [ChainId.GOERLI, ChainId.BSC, ChainId.BSC_TESTNET, ChainId.ETHEREUM, ChainId.SHIB_TESTNET]
export const bCakeSupportedChainId = [ChainId.BSC, ChainId.BSC_TESTNET, ChainId.SHIB_TESTNET]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  ...
  const fetchFarms = async (
    ...
    const masterChefAddress = masterChefAddresses[ChainId.SHIB_TESTNET]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.BSC, ChainId.ETHEREUM, ChainId.SHIB_TESTNET].includes(chainId),
  }
}
